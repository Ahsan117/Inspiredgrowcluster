...
const formatItemRow = (item, idx) => {
  ...
  lines.forEach((ln, i) => {
    if (i === 0) {
      txt += padRight(idx + 1, col.sno) +
             ...
             padLeft (Number(item.mrp || item.item.mrp).toFixed(2),  col.mrp)  +
             padLeft (Number(item.salesPrice || item.price).toFixed(2),     col.rate) +
             padLeft ((item.quantity * (item.salesPrice || item.price)).toFixed(2), col.total) + '\n';
    } else {
  ...
  const totalQuantity = order.rows.reduce((sum, item) => sum + item.quantity, 0);
        ...
        const rawTotal = order.rows.reduce((sum, item) => sum + (item.quantity * (item.salesPrice || item.price)), 0);
...
const totalSales=order.rows.reduce((sum, item) => sum + (item.quantity * (item.salesPrice || item.price)), 0);
...
// const totalDue = prevDue + netBeforeTax + taxAmt - paid - exclusiveDiscount +deliveryFee + processingFee;
  ...
  if(discountApplied > 0) 
    text += addSummaryLine('Discount Applied:', `-${(discountApplied)?.toFixed(2) || 0}`);
  text += addSummaryLine('TOTAL:', ((taxAmt || 0)+ totalSales + deliveryFee + processingFee - order.discountApplied)?.toFixed(2) || 0);
  ...
  //       text += `Payment Type: ${p.paymentNote || p.paymentType?.paymentTypeName || ""} ₹${p.amount?.toFixed(2)||0}\n`;
  //   });
  // --- Footer ---
...
const totalDue = prevDue + netBeforeTax + taxAmt + deliveryFee + processingFee;
...
<img src="./inspiredgrow.jpg" alt="logo" style="height: 40px; width: auto; margin-bottom: 8px;" />
  ...
  ${store.email}
                    ...
                    <table class="no-border">
                        <tr><td><strong>Invoice</strong></td><td class="r">#${order.orderNumber}</td></tr>
                        <tr><td><strong>Name</strong></td><td class="r">${customer.customerName || customer.name || "–"}</td></tr>
                    ...
                    <table class="no-border">
                        ...
                        ${order.discountApplied > 0 && (`<tr><td>Coupon Discount</td><td class="r">-${order.discountApplied?.toFixed(2) || 0}</td></tr>`)}
                        <tr><td><strong>Total</strong></td><td class="r"><strong>${((taxAmt || 0)+totalSales + deliveryFee + processingFee - (order.discountApplied || 0))?.toFixed(2) || 0}</strong></td></tr>
                    </table>
                    <div class="sep"></div>
...
if(loading) return <LoadingScreen />;
    ...
    <div className="w-full max-w-2xl mx-auto">
        ...
        <div className="flex justify-between mb-4 bg-white border rounded-lg shadow-md">
            <button 
                onClick={()=>{
                    Navigate("/order/view");setPrint(null);setDevice(false)
                }} 
