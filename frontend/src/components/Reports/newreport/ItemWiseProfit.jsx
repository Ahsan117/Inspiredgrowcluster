...
export default function ItemWiseProfitReport() {
  ...
  const fetchFilters = async () => {
    try {
      ...
      setOptions({
        warehouses: [{ label: "All", value: "all" }, ...wareRes.data.data.map(w => ({ label: w.warehouseName, value: w._id }))],
        suppliers:[{ label: "All", value: "all" }, ...supplierRes.data.data.map(w => ({ label: w.supplierName, value: w._id }))]
      });
    } catch (err) {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      console.log({
        warehouse:selectedWarehouse!=="all"?selectedWarehouse.map(i => i.value):options.warehouses.map(i =>{
           if(i.value!=="all"){
            return i.value
           }
        }),
        supplier:selectedSupplier!=="all"?selectedSupplier.map(i=>i.label):options.suppliers.map(i=>i.label),
        search:search
      })
            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
              <div>
                ...
                <Select
                  ...
                  value={options.warehouses.find((w) => w.value === selectedWarehouse)}
              ...
              <div>
                ...
                <Select
                  ...
                  value={options.suppliers.find((w) => w.value === selectedSupplier)}
                  onChange={(e) => setSelectedSupplier(e.label)}
                />
              </div>
              <div>
