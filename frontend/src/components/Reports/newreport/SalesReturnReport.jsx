...
export default function StockReport() {
  ...
  const fetchItems = async () => {
    try {
      ...
      const response = await axios.get(`${link}/api/reports/sales-return-report`, {
  ...
  const exportToPDF = () => {
    ...
    autoTable(doc, {
      head: [["#","Refernce No", "Return Code", "Return Date", "Customer", "Warehouse", "Total"]],
...
{loading && <LoadingScreen />}
        <div className="flex flex-col w-full max-h-screen min-h-screen p-6 overflow-y-auto">
          <header className="flex flex-col items-start mb-4 sm:flex-row sm:items-center">
            <h1 className="text-2xl font-semibold text-gray-800">Sales Return Report</h1>
          </header>
              ...
              <table className="w-full text-sm border">
                <thead className="text-white bg-cyan-600">
                  <tr>
                    ...
                    <th className="p-2">Return Code</th>
                ...
                <tbody>
                  {allItems.map((item, i) => (
                    <tr key={i} className="border-b hover:bg-gray-50"
                      ...
                      <td className="p-2 text-center">{item.referenceNo}</td>
                      <td className="p-2 text-center">{new Date(item.returnDate).toLocaleDateString()}</td>
                      <td className="p-2 text-center">{item.returnCode}</td>
                      <td className="p-2 text-center">{item.customer?.customerName}</td>
                      <td className="p-2 text-center">{item.warehouse?.warehouseName}</td>
                      <td className="p-2 text-center">{item.totalRefund}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
