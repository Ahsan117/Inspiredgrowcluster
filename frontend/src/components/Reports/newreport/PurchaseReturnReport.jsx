...
export default function StockReport() {
  ...
  const fetchItems = async () => {
    try {
      ...
      if (selectedSuppliers.length > 0) {
        params.supplierId = selectedSuppliers.map(i => i.value);
      }

      const response = await axios.get(`${link}/api/reports/purchase-return-report`, {
  ...
  const exportToPDF = () => {
    ...
    autoTable(doc, {
      head: [["#", "Purchase Code", "Purchase Date", "Supplier", "Warehouse", "Total"]],
      ...
      <Sidebar isSidebarOpen={isSidebarOpen} />
        ...
        <div className="flex flex-col w-full max-h-screen min-h-screen p-6 overflow-y-auto">
          <header className="flex flex-col items-start mb-4 sm:flex-row sm:items-center">
            <h1 className="text-2xl font-semibold text-gray-800">Purchase Return Report</h1>
          </header>
              ...
              <table className="w-full text-sm border">
                <thead className="text-white bg-cyan-600">
                  <tr>
                    ...
                    <th className="p-2">Purchase Code</th>
                ...
                <tbody>
                  {allItems.map((item, i) => (
                    <tr key={i} className="border-b hover:bg-gray-50"
                      onClick={() => navigate(`/purchase-return-detail/${item._id}`)}
                      >
                      <td className="p-2 text-center">{i + 1}</td>
                      <td className="p-2 text-center">{new Date(item.purchaseDate).toLocaleDateString()}</td>
                      <td className="p-2 text-center">{item.purchaseCode}</td>
                      <td className="p-2 text-center">{item.supplier?.supplierName}</td>
                      <td className="p-2 text-center">{item.warehouse?.warehouseName}</td>
                      <td className="p-2 text-center">{item.grandTotal?.toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
