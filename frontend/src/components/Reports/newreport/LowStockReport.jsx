...
export default function ItemWiseProfitReport() {
  ...
  const fetchItems = async () => {
    try {
      ...
      //   search:search
      // })
      const response = await axios.get(`${link}/api/reports/lowstock-report`, {
  ...
  const exportToExcel = () => {
    ...
    const fileName = `LowStockITem_Report_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  const exportToPDF = () => {
    ...
    doc.save(`LowStockItem_Report_${new Date().getTime()}.pdf`);
  };
  ...
  loading && <LoadingScreen />
        ...
        <div className="flex flex-col w-full max-h-screen min-h-screen p-6 overflow-y-auto">
          ...
          <header className="flex flex-col items-start justify-between mb-4 sm:flex-row sm:items-center">
            <h1 className="text-2xl font-semibold text-gray-800">LowStock Item Report</h1>
            <nav className="flex items-center gap-2 text-sm text-gray-600">
              <NavLink to="/dashboard" className="flex items-center hover:text-cyan-600">
                <FaTachometerAlt className="mr-2" /> Home
              </NavLink>
              <span>&gt;</span>
              <span className="text-cyan-600">LowStock Report</span>
            </nav>
          </header>
