...
class OrderService {
  ...
  static Map<String, dynamic> parseOrderData(Map<String, dynamic> rawOrder) {
    try {
      // Debug the source of the data
      print('=== PARSING ORDER DATA ===');
      print('Raw order keys: ${rawOrder.keys.toList()}');
      ...
      print('Processing order: $orderNumber');
      ...
      // ITEMS PARSING - This is where the issue likely is
      final rawItems =
          rawOrder['items'] ??
              rawOrder['orderItems'] ??
              rawOrder['products'] ??
      ...
      print('=== ITEMS PARSING DEBUG ===');
      print('Raw items type: ${rawItems.runtimeType}');
      print(
        'Raw items length: ${rawItems is List ? rawItems.length : 'Not a list'}',
      );

      if (rawItems is List && rawItems.isNotEmpty) {
        print('First raw item structure: ${rawItems[0]}');
        print(
          'First raw item keys: ${rawItems[0] is Map ? (rawItems[0] as Map).keys.toList() : 'Not a map'}',
        );
      }
        ...
        parsedItems =
            rawItems.map((item) {
              try {
                ...
                print('Item raw structure: $itemMap');
                print('Item data keys: ${itemDataMap.keys.toList()}');
                print('Item name: ${itemDataMap['itemName'] ?? 'Unknown'}');
                print(
                  'Item ID: ${itemDataMap['_id'] ?? itemDataMap['id'] ?? 'No ID'}',
                );

                // Check for itemImages specifically (same as ItemListPage)
                if (itemDataMap.containsKey('itemImages')) {
                  print('Found itemImages field: ${itemDataMap['itemImages']}');
                }
                ...
                // Image URL extraction using the same method as ItemListPage
                ...
                return {
                  ...
                  'image': imageUrl, // Add this for compatibility
                  'brand':
                  ...
                  'itemImages':
                  itemDataMap['itemImages'], // Keep the original itemImages field
                };
              } catch (e) {
      ...
      print('=== FINAL ITEMS SUMMARY ===');
      print('Parsed items count: ${parsedItems.length}');
      for (int i = 0; i < parsedItems.length; i++) {
        print(
          'Item $i: ${parsedItems[i]['itemName']} - Image: ${parsedItems[i]['productImage']}',
        );
      }
      print('=== END ITEMS SUMMARY ===\n');

      // Rest of the parsing logic...
      // Extract ALL billing fields to ensure complete billing breakdown is available
      // This ensures all fields are passed to the UI even if they're 0
      ...
      // Extract shippingFee and deliveryCharge separately (they might be different)
      /*final shippingFee = safeToDouble(
        rawOrder['shippingFee'] ?? rawOrder['shippingCharge'],
      );*/
      final deliveryCharge = safeToDouble(
        rawOrder['deliveryCharge'] ?? rawOrder['deliveryFee'],
      );

      // Extract processingFee separately (might be different from platformCharge)
      final processingFee = safeToDouble(
        rawOrder['processingFee'] ?? rawOrder['paymentProcessingFee'],
      );

      // Extract platformCharge (might be different from processingFee)
      ...
      final parsedOrder = {
        ...
        'tax': tax,
        'deliveryCharge': deliveryCharge, 
        'processingFee': processingFee, // Add processingFee separately
        'platformCharge': platformCharge, // Add platformCharge separately
        'discountApplied': discountApplied,
        'tip': tip, // Add tip
        'donation': donation, // Add donation
        'finalAmount': finalAmount,
        'grandTotal': finalAmount,
        'total': finalAmount,
        'totalAmount': finalAmount, // Add totalAmount as well
        // NEW: Include delivery slot information if present
      ...
      print(
        '✅ Successfully parsed order: ${parsedOrder['orderNumber']} with ${parsedItems.length} items',
      );
      return parsedOrder;
    } catch (e) {
      print('❌ Error parsing order data: $e');
      print('Raw order data: $rawOrder');
