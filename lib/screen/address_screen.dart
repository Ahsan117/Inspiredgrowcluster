...
class AddressScreen extends StatefulWidget {
  final bool isSelectionMode; // New parameter to determine if selecting for location

  const AddressScreen({
    Key? key,
    this.isSelectionMode = false, // Default to false for backwards compatibility
  }) : super(key: key);
...
class _AddressScreenState extends State<AddressScreen> {
  ...
  // New method to handle address selection for location
  Future<void> _selectAddress(Address address) async {
    ...
    try {
      // Check zone using LocationProvider
      ...
      if (result['success'] == true && result['isServiceable'] == true) {
        // Show success dialog with delivery info
        _showSuccessDialog(
          'Address Selected',
          'Delivery Zone: ${result['zoneName']}\n'
              'Estimated Delivery: ${result['deliveryTime']} mins\n'
              'Delivery Fee: ₹${result['deliveryFee']?.toStringAsFixed(0) ?? '0'}\n'
              'Min Order: ₹${result['minOrder']?.toStringAsFixed(0) ?? '0'}',
          onConfirm: () {
        ...
        // Not serviceable
        _showNotServiceableDialog(result['message'] ?? 'Not serviceable');
      }
    } catch (e) {
      if (!mounted) return;
      Navigator.pop(context); // Close loading dialog
...
// In address_screen.dart, update the success dialog's OK button:
  ...
  void _showNotServiceableDialog(String message) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: const Row(
          children: [
            Icon(Icons.info_outline, color: AppColors.errorRed, size: 28),
            SizedBox(width: 12),
            Text('Not Serviceable', style: TextStyle(fontWeight: FontWeight.w600)),
          ],
        ),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              'We\'re expanding our delivery areas. Check back soon!',
              style: TextStyle(
                fontSize: 14,
                color: Colors.grey[600],
